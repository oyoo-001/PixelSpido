...
import { Zap, Mail, Lock, User, Loader2, Shield } from "lucide-react";
...
export default function Login() {
  ...
  const handle2FAVerify = async (e) => {
    e.preventDefault();
    if (!twoFactorCode) {
      toast.error("Missing Code", { description: "Please enter the 6-digit code" });
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md border border-border rounded-lg bg-card p-6 shadow-sm">
        ...
        <Dialog open={show2FA} onOpenChange={setShow2FA}>
          <DialogContent className="sm:max-w-md">
            ...
            <form onSubmit={handle2FAVerify} className="space-y-4">
              ...
              <Button type="submit" className="w-full" disabled={verifying2FA || twoFactorCode.length !== 6}>
                {verifying2FA ? (
                  <>
    ...
    </div><h1 className="text-3xl font-bold tracking-tight">PixelSpido</h1><p className="text-sm text-muted-foreground mt-2">
        {isLogin ? "Sign in to your account" : "Create your account"}
      </p></>
        <div>
        ...
        <><form onSubmit={handleSubmit} className="space-y-4">
          ...
          </form><p className="text-center text-sm text-muted-foreground">
              {isLogin ? "Don't have an account?" : "Already have an account?"}{" "}
              <button
                type="button"
                onClick={() => setIsLogin(!isLogin)}
